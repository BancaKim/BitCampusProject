...
class PerClinetThread extends Thread {
    ...
    static List<PrintWriter> list = Collections.synchronizedList(new ArrayList<PrintWriter>());  //서버에 연결된 모든 클라이언트에 메시지를 보내는데 사용
    ...
    PerClinetThread(Socket socket) {
        this.socket= socket;
        try {
            writer = new PrintWriter(socket.getOutputStream());   // getOutputSream을 반환하여 네트워크를 통해 바이트 데이터 전송
            list.add(writer);
        }
        catch (Exception e) {
    ...
    public void run() {
        ...
        try {
            BufferedReader reader = new BufferedReader(
                new InputStreamReader(socket.getInputStream()));     // 소켓으로부터 데이터를 받아들이기 위한 입력 스트림 설정 과정
